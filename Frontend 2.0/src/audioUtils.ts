...
export class AudioPlayer {
  ...
  resetStream(): void {
    ...
    if (this.useMediaSource) {
      ...
      // Properly clean up old SourceBuffer before creating a new MediaSource
      if (this.sourceBuffer) {
        try {
          if (this.sourceBuffer.updating) {
            this.sourceBuffer.abort();
          }
          // Remove from old MediaSource if still open
      ...
      // End the old MediaSource stream if still open
      if (this.mediaSource && this.mediaSource.readyState === 'open') {
        try {
      ...
      // Revoke old object URL to free resources
      ...
      const currentMediaSource = this.mediaSource; // capture for closure
      ...
      this.mediaSource.addEventListener('sourceopen', () => {
        // Guard: only process if this is still the active MediaSource
        if (currentMediaSource !== this.mediaSource) return;
        ...
        try {
          ...
          // Flush any chunks that arrived before sourceopen fired
          this.flushQueue();
        } catch (e) {
